...
def preprocess(images: 'np.array') -> 'np.array':
    """
    Input images is a (n_samples, 28, 28) matrix.
    To apply a classifier on this data, we need to flatten the image, i.e.,
    turn the data in a (samples, n_dim) matrix, where n_dim = 28*28

    :param images: a 3D matrix with shape (n_samples, 28, 28)
    :return: a flattened image matrix of shape (n_samples, 28*28)
    """
